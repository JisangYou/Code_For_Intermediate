package duplicate_observed_data.before;
 ...
 * 방법: 도메인 데이터는 GUI 컨트롤 안에서만 사용 가능한데, 도메인 메서드가 그 데이터에 접근해야 할 땐 그 데이터를 도메인 객체로 복사하고,
 * 양측의 데이터를 동기화하는 관측 인터페이스 observer를 작성한다.
 *
 * use case: 계층구조가 체계적인 시스템은 비즈니스 로직과 사용자 인터페이스 처리코드가 분리.
 * 인터페이스와 비즈니스를 분리함으로써 유지보수의 향상과 업무 분할 용이성
 */
